...
# script to cut ribosomal 16S RNA sequences as annotated by barrnap out of contigs (as long as the contigs are still at least 1 nt)
...
# writes 2 outputs: - a fasta file with the all contigs (whole contigs without rRNA, cut contigs with rRNA removed)
#		    - a table with the length of the removed sequences

# written by Anna Heintz-Buschart (adjusted in August 2016 to remove length cutoff)
